...
def plot_results(results_dict, task, fig_path):
    ...
    for t in tsteps:
        ...
        for model_name, result_dict in results_dict.items():
            ...
            if t == 0:
                rects = ax.bar(width * multiplier, mean, width, color=colors[cc],
                            yerr=std, label=model_name, edgecolor='black')
                ax.bar_label(rects, padding=3, fontsize=11)
            else:
                rects = ax.bar(width * multiplier, mean, width, color=colors[cc],
                            yerr=std, edgecolor='black')
                ax.bar_label(rects, padding=3, fontsize=11)
            multiplier += 1
            cc += 1
        multiplier += 1
    ...
    offsets = [i*width*len(results_dict) + (i-0.5)*width  for i in range(len(tsteps))]
    ...
    plt.savefig(f"{fig_path}_HELLO.png", dpi=1000)
