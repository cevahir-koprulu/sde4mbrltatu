...
def plot_trajectories(task, task_id, fields_to_plot, pred_traj, start_time, end_time, data_horizon, traj_gt, fig_path):
    per_subplot_figsize = (3,3)
    num_subfigures = len(fields_to_plot) + 1
    num_cols = min(num_subfigures, MAX_NUM_COLS)
    num_rows = math.ceil(num_subfigures // num_cols)

    print(f"num_cols: {num_cols} || num_rows: {num_rows}")

    first_segment = (start_time // data_horizon)
    last_segment = (end_time // data_horizon)
    ...
    num_plotted_model = 0
    label_column = num_cols//2 # 1
    for model_name, pred in pred_traj.items():
        prediction = pred["pred"]
        error = pred["error"]
        color = pred["color"]

        time_pred = prediction["time_pred"][first_segment:last_segment]
        # total_error_mean = error["total_error_mean"][first_segment:last_segment].ravel()
        # total_error_std = error["total_error_std"][first_segment:last_segment].ravel()
        total_error_mean = np.mean(error["total_error"][first_segment:last_segment], axis=1).ravel()
        total_error_std = np.std(error["total_error"][first_segment:last_segment], axis=1).ravel()
        axs[0].plot(time_pred.ravel(),
                    total_error_mean, 
                    c=color, linestyle="-"
        )
        axs[0].fill_between(time_pred.ravel(),
                            total_error_mean - total_error_std,
                            total_error_mean + total_error_std,
        ...
        label_plot = True
        for i, field_name in enumerate(fields_to_plot):
            if num_plotted_model == 0:
                axs[i+1].plot(traj_gt["time"][start_time:end_time], 
                        traj_gt[field_name][start_time:end_time], 
                        label="GT" if label_column == (i+1)%num_cols else None,
                        c="k", linestyle="--",
                        zorder=1000
                )

            pred_field = prediction[field_name + "_pred"][first_segment:last_segment]
            for t, ys in zip(time_pred, pred_field):
                for y in ys:
                    label = None
                    if label_column == (i+1)%num_cols and label_plot:
                        label = model_name
                        label_plot = False
                    axs[i+1].plot(t, y, label=label, c=color, linestyle="-")
                axs[i+1].set_ylabel(field_name)
                axs[i+1].grid(True)
                if i > (num_subfigures-num_cols) - 1 and num_cols//2 == (i+1)%num_cols:
                    axs[i+1].set_xlabel('time')
        num_plotted_model += 1
    axs[label_column].legend(fontsize=plt.rcParams['font.size']*0.7, ncol=1)
    plt.savefig(f"{fig_path}.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--end-time", type=int, default=151)
    ...
    plot_trajectories(
        ...
        data_horizon=args.data_horizon,
