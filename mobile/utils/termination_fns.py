...
def hopper_reward_fn(obs, act, next_obs):
    assert len(obs.shape) == len(next_obs.shape) == len(act.shape) == 2
    alive_bonus = 1.0
    # reward = obs[...,5]
...
def halfcheetah_reward_fn(obs, act, next_obs):
    assert len(obs.shape) == len(next_obs.shape) == len(act.shape) == 2
...
def walker2d_reward_fn(obs, act, next_obs):
    assert len(obs.shape) == len(next_obs.shape) == len(act.shape) == 2
    alive_bonus = 1.0
...
def get_reward_fn(task):
    if 'hopper' in task:
        return hopper_reward_fn
    elif 'halfcheetah' in task:
        return halfcheetah_reward_fn
    elif 'walker2d' in task:
        return walker2d_reward_fn
    else:
