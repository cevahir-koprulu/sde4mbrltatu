...
def analyze_model(dataset, model_names, hr, num_extra_steps, num_sample, num_traj_show, use_train=False, seed=10):
    ...
    for model_idx, model_name in enumerate(model_names):
        ...
        _xres, _tres, err_res = n_steps_analysis(curr_traj_y, curr_traj_u, base_model_fn_jit, t_model, TIMESTEP_ENV, traj_time_evol)
        ...
        # Now ler's plot the error for each state
        _error_full = err_res
        if indx2show is None:
            num_traj_show = min(num_traj_show, len(err_res))
            indx2show = np.random.choice(len(err_res), size=num_traj_show, replace=False)
            # print('INFO: Showing the error for the following trajectories: ', indx2show)
        if fig_err is None:
            fig_err, axs_err = [], []
            for _i in range(len(indx2show)):
                _fig, _axs = plt.subplots(num_rows,num_cols,figsize=TOTAL_FIG_SIZE,sharex=True)
                _axs = _axs.flatten()
                fig_err.append(_fig)
                axs_err.append(_axs)

        for _indx, axs in zip(indx2show, axs_err):
            _err_array = _error_full[_indx]
            for i in range(num_states):
                # state_error = np.array([_v[:,i] for _v in _error_full])
                # state_error = np.mean(state_error, axis=0)
                state_error = _err_array[:,i]
        ...
        #     norm_error = np.array([_v[:,4] for _v in _error_full])
        #     norm_error = np.mean(norm_error, axis=0)

        #     # Now we show the norm and standard deviation of the error
        #     axs[4].plot(_err_array[:,-1], norm_error, color=curr_color, label=_model_name)
        #     axs[4].set_xlabel('Time (s)')
        #     axs[4].set_ylabel('Norm of the error')
        #     axs[4].grid(True)
        #     # Now we show the norm and standard deviation of the error
        #     axs[5].plot(_err_array[:,-1], _err_array[:,5], color=curr_color, label=_model_name)
        #     axs[5].set_xlabel('Time (s)')
        #     axs[5].set_ylabel('Standard deviation of the error')
        #     axs[5].grid(True)
...
if __name__ == '__main__':
    ...
    # JAX_PLATFORM_NAME=cpu python train_n_analyze_models.py --analyze --dataset halfcheetah-random-v2 --hr 30 --use_train --seed 55 --model_names hc_rand_v2_dsc0.3_hr-5_dt-0.05_sde.pkl hc_rand_v2_dsc0.1_hr-5_dt-0.05_sde.pkl
    # Argument parser
    ...
    if args.analyze:
        # analyze_model(dataset, model_names, hr, dt, num_sample, num_traj_show, use_train=False, seed=10)
        analyze_model(args.dataset, args.model_names, args.hr, args.num_extra_steps, args.nsample, args.ntraj, args.use_train, args.seed)